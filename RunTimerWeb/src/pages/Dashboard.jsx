  ...
  useEffect(() => {
    ...
    const generateNotifications = (compData) => {
      ...
      compData.forEach((c) => {
        ...
        newNotifications.push({
          id: c.id,
          title: c.name || c.nombre || "Competencia",
          message: `Tienes ${c.name || c.nombre} el día ${fechaTexto} en ${c.location || c.lugar || "sin lugar"}`,
        });
      });
    ...
    >
      ...
      <aside className="w-72 bg-white dark:bg-zinc-950 border-r border-gray-200 dark:border-zinc-900 p-6 hidden md:flex flex-col transition-colors duration-300">
        ...
        <nav className="flex flex-col gap-3">
          ...
          <button onClick={() => goTo("/competitions")} className="flex items-center gap-4 px-5 py-4 rounded-2xl bg-gray-100/70 dark:bg-zinc-900 hover:bg-gray-200/80 dark:hover:bg-zinc-800 text-gray-700 dark:text-gray-300 transition font-medium text-left">
            <Trophy size={22} /> Competencias
          </button>
          <button onClick={() => goTo("/results")} className="flex items-center gap-4 px-5 py-4 rounded-2xl bg-gray-100/70 dark:bg-zinc-900 hover:bg-gray-200/80 dark:hover:bg-zinc-800 text-gray-700 dark:text-gray-300 transition font-medium text-left">
            <Activity size={22} /> Resultados
          </button>
          <button onClick={() => goTo("/profile")} className="flex items-center gap-4 px-5 py-4 rounded-2xl bg-gray-100/70 dark:bg-zinc-900 hover:bg-gray-200/80 dark:hover:bg-zinc-800 text-gray-700 dark:text-gray-300 transition font-medium text-left">
            <Users size={22} /> Perfil
          </button>
          <button onClick={() => goTo("/settings")} className="flex items-center gap-4 px-5 py-4 rounded-2xl bg-gray-100/70 dark:bg-zinc-900 hover:bg-gray-200/80 dark:hover:bg-zinc-800 text-gray-700 dark:text-gray-300 transition font-medium text-left">
            <Settings size={22} /> Configuración
          </button>
        </nav>

        <div className="mt-auto pt-6 flex flex-col gap-4">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => goTo("/podium")} className="w-full relative overflow-hidden rounded-[28px] p-5 text-left border border-red-500/20 bg-gradient-to-br from-red-500/15 via-zinc-900/10 to-transparent dark:from-red-500/10 dark:via-zinc-900/50 dark:to-zinc-950 hover:border-red-500/40 transition-all duration-300 group">
            ...
            <div className="relative flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-[0.18em] text-red-500 flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" /> Live</span>
                </div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Podio en Vivo</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">Visualiza posiciones, tiempos y ganadores en tiempo real.</p>
              </div>
            </div>
          </motion.button>
          <button onClick={handleLogout} className="w-full bg-zinc-100 hover:bg-red-500 dark:bg-zinc-900 dark:hover:bg-red-600 text-gray-700 dark:text-zinc-300 hover:text-white dark:hover:text-white transition-all py-4 rounded-2xl font-semibold border border-transparent dark:border-zinc-800/60">
            Cerrar sesión
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto relative">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-10">
          ...
          <div className="flex items-center gap-4 relative">
            ...
            {showNotifications && (
              <div className="absolute top-20 right-0 w-[360px] bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-3xl shadow-xl z-50 overflow-hidden">
                <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-zinc-800">
                  <h2 className="font-bold text-lg">Notificaciones</h2>
                  <button onClick={() => setShowNotifications(false)}><X size={20} className="text-gray-400 hover:text-red-500" /></button>
                </div>
                <div className="max-h-[350px] overflow-y-auto">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div key={n.id} className="p-4 flex justify-between items-start border-b border-gray-50 dark:border-zinc-900 hover:bg-gray-50 dark:hover:bg-zinc-900 transition">
                        <div>
                          <p className="font-bold text-black dark:text-white">{n.title}</p>
                          <p className="text-sm text-gray-500">{n.message}</p>
                        </div>
                        <X size={18} className="cursor-pointer text-gray-400 hover:text-red-500 ml-2 mt-1" onClick={() => setNotifications((prev) => prev.filter((x) => x.id !== n.id))} />
                      </div>
                    ))
                  )}
                </div>
            ...
            <button onClick={() => goTo("/Editprofile")} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl px-5 py-3 flex items-center gap-4 hover:bg-gray-100 dark:hover:bg-zinc-800 transition cursor-pointer shadow-sm text-left">
              <img src={userData?.photoUrl || currentUser?.photoURL || "https://ui-avatars.com/api/?name=RunTimer"} alt="Perfil" className="w-12 h-12 rounded-full object-cover border border-gray-200 dark:border-zinc-700" />
              <div>
     ...
     {/* STATS */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm flex flex-col justify-between">
            <div>
              <h2 className="text-gray-500 dark:text-gray-400 font-medium">Soporte RUNtimer</h2>
              <p className="text-sm text-gray-400 mt-1">¿Alguna duda o mejora?</p>
            </div>
            <button 
              onClick={() => goTo("/contacto-creadores")} 
        ...
        <section className="bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-900 rounded-[32px] p-6 md:p-8 transition-colors duration-300 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Carreras Registradas</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Selecciona una competencia para ver detalles y clasificaciones.</p>
            </div>
            <span className="bg-red-500/10 text-red-500 px-3 py-1.5 rounded-xl font-bold text-xs tracking-wider uppercase">{competitions.length} Total</span>
          </div>
          {competitions.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-3">
              {competitions.map((competition) => (
                <motion.div key={competition.id} whileHover={{ x: 4 }} onClick={() => handleSelectCompetition(competition.id)} className="w-full flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-zinc-900/50 hover:bg-gray-100 dark:hover:bg-zinc-900 border border-gray-100 dark:border-zinc-900 cursor-pointer transition-all duration-200 group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-900 dark:text-white text-base">{competition.name || competition.nombre || "Carrera sin nombre"}</h3>
                      <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                        <span className="flex items-center gap-1"><Calendar size={12} /> {formatCompetitionDate(competition.date || competition.fecha)}</span>
                        {competition.type && (<span className="bg-gray-200/60 dark:bg-zinc-800 px-2 py-0.5 rounded-md">{competition.type}</span>)}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-semibold text-gray-400 dark:text-zinc-500 group-hover:text-red-500 dark:group-hover:text-red-400 transition-colors hidden sm:inline">Ver carrera</span>
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm transition-colors duration-300">
      <div className="flex justify-between mb-5">
        <div className="w-14 h-14 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-500">{icon}</div>
